/**
 * ClipForge Dashboard — Super Series pure logic.
 * EXACT port of bot/src/super_series.js (itself paired 1:1 with
 * pipeline/plan/super_series.py — the validators on both sides must return
 * the same accept/reject decisions and the same error strings for the same
 * input, pinned by pipeline/tests/test_super_series.py).
 *
 * DESIGN (feature-01)
 * -------------------
 * The operator submits ONE super-plan document (ARCHITECTURE.md §7.5): a
 * ``parts`` array of ordinary, self-contained per-part production.json
 * documents plus a shared ``series_id``. This module:
 *   1. validates the WHOLE document up front (validateSuperPlan);
 *   2. slices out exactly one part at a time (sliceSuperPart) into an
 *      ordinary §7.3 single-part production.json and a §7.1
 *      stage-a-request body synthesized with the EXISTING series
 *      continuation helpers (series.js nextPartRequestBody / nextPartJobId);
 *   3. decides the queue's next move from durable repo state alone
 *      (superQueueAdvance) — a pure decision function, so the browser (and
 *      the still-live bot cron, and later the task-12 scheduled workflow)
 *      all advance the same queue without double-dispatching.
 *
 * HALT-AND-RESUME: when a part's Stage B run reaches state "error" the
 * queue HALTS; when that part later reaches "complete" (via the ordinary
 * Restart Stage B flow) the same decision function returns the next part to
 * queue — resumption needs no separate trigger.
 */

import { validateProductionPlan } from './plan.js';
import { buildSeriesContext, nextPartJobId, nextPartRequestBody, isValidJobId } from './series.js';
...
export function sliceSuperPart(document, partIndex, jobId) {
  ...
  // every client flows through — appends the marker when the author-supplied
  // title lacks it. "Title — Part N" matches the banner convention of an
  // ordinary series part ("... Part N"); titles that already name a part
 ...
 * lives in the anchor job's directory so the existing cleanup/TTL machinery
 * expires it together with the job, and so a restart loses nothing.
 */
 ...
 * { part, summary } entries from already-completed spawned parts.
 */
...
/**
 * REPO-BASED anchor test: a job is an ACTIVE Super Series anchor iff its
 * super-plan.json holds a well-formed queue record whose parts are not all
 * spawned. Answerable purely by reading a file already in the repo — no D1.
 */
export function isSuperSeriesAnchor(document) {
  const state = document;
  if (!state || typeof state !== 'object' || Array.isArray(state)) return false;
  if (!state.plan || !Array.isArray(state.plan.parts)) return false;
  const totalParts = Number(state.total_parts) || state.plan.parts.length;
  const spawned = Array.isArray(state.spawned) ? state.spawned : [];
  return spawned.length < totalParts; // all parts spawned => done/inactive
}
