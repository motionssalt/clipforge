#!/usr/bin/env python3
"""ClipForge Gemini capability diagnostic (ARCHITECTURE.md §12).

Port of ``_legacy/scripts/gemini_api_capability_diagnostic.py`` onto the
package layout. Safely verifies whether the repository's configured Gemini
keys support Automatic Mode. It uses the official ``google-genai`` SDK and
tests a minimal native function-call request that also contains an inline
PNG. It never prints or writes raw keys, provider request bodies, response
text, cookies, or Google-project identifiers. The JSON report contains only
key fingerprints, selected model IDs, boolean capabilities, HTTP-like error
codes, and bounded redacted error labels.

It cannot determine whether two API keys share a Google Cloud project: Gemini
Developer API responses do not expose project ownership. That association
must be checked in the key owner's AI Studio Projects/API Keys views.

Model candidates are shared with the Automatic-mode planner
(``pipeline.plan.automatic``) so the diagnostic probes exactly the models the
pipeline would use.

Env:
    GEMINI_API_KEYS   comma/newline separated API keys (never printed)

Usage:
    python -m pipeline.diagnostics.gemini --output report.json
"""
...
# Probe the same models Automatic mode would use, most-preferred first.
MODEL_CANDIDATES = (DEFAULT_PRIMARY_MODEL, *DEFAULT_FALLBACK_MODELS)
# A valid 1x1 PNG. It is deliberately content-free and only establishes that
# the selected model/key accepts an inline image part alongside a function
# tool.
    ...
    contents = types.Content(
        ...
        parts=[
            types.Part.from_text(
                text=(
                    "This is a ClipForge capability probe. Inspect the attached 1x1 PNG and call "
