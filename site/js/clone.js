...
// ------------------------------------------------------------------------ //
// Clone sync (port of bot/src/clone-sync.js — bug-21/bug-65 semantics)      //
// "Sync from source" for Settings → GitHub clone: compare the connected     //
// clone against motionssalt/clipforge@main via .clipforge-sync.json and     //
// apply the diff as ONE git commit based on the clone's current main HEAD   //
// (untouched clone-local paths — branding/, jobs/, audio-library/, keys/    //
// accounts — are preserved automatically).                                  //
// ------------------------------------------------------------------------ //

export const SYNC_PATH = '.clipforge-sync.json';
...
async function readSyncFile(credentials, repo) {
  ...
  try {
    ...
    if (!file || typeof file.content !== 'string') return null;
    const parsed = JSON.parse(b64decode(file.content.replace(/\n/g, '')));
...
export async function checkCloneUpdates(credentials) {
  ...
  if (!repo) throw new Error('Not connected to a clone.');
  ...
  if (!synced || !synced.sha) {
    ...
    return {
      upToDate: false, sourceSha, syncedSha: '', syncedAt: '',
...
/**
 * Apply the changeset returned by checkCloneUpdates to the clone — a single
 * git tree/commit against the clone's current main HEAD as base_tree, so
 * untouched clone-local paths are preserved automatically.
 */
export async function applyCloneUpdates(credentials, plan) {
  ...
  const tree = await githubRequest(
    ...
    { method: 'POST', body: { tree: entries, base_tree: baseTreeSha } }
  );
